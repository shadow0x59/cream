...
import {ExtendedRequest, MiddlewareDataCollection, MiddlewareDataCollections,} from '../ExpressMiddleware/ExpressMiddleware';
...
export function ExpressCall<T extends ExpressModule>(
    relativePath: string, httpMethod: HttpMethod = HttpMethod.GET) {
  return function(
      target: T, propertyName: string, descriptor: PropertyDescriptor) {
    let method = descriptor.value!;
    let newMethod = (thisArg: ExpressModule) => async function(
        req: ExtendedRequest, res: Response, next: NextFunction) {
      let args = [];
      let bodyAssocs: ParameterProps =
          Reflect.getOwnMetadata(BODY_METADATA_KEY, target, propertyName);
      if (bodyAssocs) {
      ...
      let paramAssoc: ParameterProps =
          Reflect.getOwnMetadata(PARAMS_METADATA_KEY, target, propertyName);
      if (paramAssoc) {
      ...
      let headerMappings: ParameterProps =
          Reflect.getOwnMetadata(HEADERS_METADATA_KEY, target, propertyName);
      ...
      let middlewareAssoc: MiddlewareParameterProps =
          Reflect.getOwnMetadata(MIDDLEWARE_METADATA_KEY, target, propertyName);
      if (middlewareAssoc) {
...
export function
ExpressController<T extends {new (...args: any[]): ExpressModule}>(
    baseRoute: string) {
...
export function BodyField(fieldName: string) {
  return function(
    ...
    Reflect.defineMetadata(
        BODY_METADATA_KEY, existingRequiredParameters, target, propertyKey);
  };
}
...
export function UrlParameter(fieldName: string) {
  return function(
    ...
    existingRequiredParameters.push(
        new ParameterProp(parameterIndex, fieldName));
    Reflect.defineMetadata(
        PARAMS_METADATA_KEY, existingRequiredParameters, target, propertyKey);
  };
}
...
export function Header(headerName: string) {
  return function(
    ...
    existingHeaderMappings.push(new ParameterProp(parameterIndex, headerName));
    Reflect.defineMetadata(
        HEADERS_METADATA_KEY, existingHeaderMappings, target, propertyKey);
  };
}
