...
export function MiddlewareData(
    collectionName: string = 'default', dataName: string = '$') {
  return function(
      target: Object, propertyKey: string|symbol, parameterIndex: number) {
    let existingRequiredParameters: MiddlewareParameterProps =
        Reflect.getOwnMetadata(MIDDLEWARE_METADATA_KEY, target, propertyKey) ||
        [];
    existingRequiredParameters.push(
        new MiddlewareParameterProp(parameterIndex, dataName, collectionName));
    Reflect.defineMetadata(
        MIDDLEWARE_METADATA_KEY, existingRequiredParameters, target,
        propertyKey);
  };
}
